...
Pour insÃ©rer des images provenant d'autres logiciels que LaTeX, utilisez le paquet `graphicx`, qui fournit la commande `\includegraphics`.
...
Vous pouvez inclure des fichiers EPS, PNG, JPG et PDF. Si vous ne donnez pas l'extension du nom de fichier, `graphicx` essaiera de la deviner. Mais si vous disposez de plusieurs versions d'une image, vous pouvez la prÃ©ciser, par exemple, `example-image.png`.

Vous pouvez remarquer que nous avons utilisÃ© ici un nouvel environnement, `center`, pour centrer l'image horizontalement sur la page. [Un peu plus tard](lesson-11), nous parlerons plus en dÃ©tail de l'espacement et du positionnement.


## Modifier l'apparence des images

La commande `\includegraphics` a de nombreuses options pour contrÃ´ler la taille et la forme des images incluses et pour rogner l'image insÃ©rÃ©e. Certaines de ces options sont trÃ¨s utilisÃ©es, il est donc important de les connaÃ®tre.

Les choses les plus Ã©videntes Ã  dÃ©finir sont la largeur de l'image (`width`) et sa hauteur (`height`), qui sont souvent donnÃ©es par rapport Ã  la largeur du texte (`\textwidth`) ou Ã  sa hauteur (`\textheight`). LaTeX mettra automatiquement l'image Ã  l'Ã©chelle pour que son rapport hauteur/largeur reste correct.
...
Vous pouvez Ã©galement mettre Ã  l'Ã©chelle les images avec `scale`, ou les faire pivoter avec `angle`. L'autre chose que vous pouvez faire est de dÃ©couper (`clip`) ou de rogner (`trim`) une image.
...
Traditionnellement, dans la composition de documents, et en particulier pour les documents techniques, les images et les tableaux peuvent Ãªtre dÃ©placÃ©s une ou quelques pages plus loin dans le document, ou regroupÃ©s Ã  plusieurs sur une page, pour mieux utiliser l'espace disponible et ne pas laisser vides de grands morceaux de pages. Ce type d'image ou de tableau s'appelle un _flottant_.
...
Si l'on n'utilisait pas le mÃ©canisme des flottants, l'image appraÃ®trait juste sous le texte `Emplacement initial`. Mais quand on permet Ã  l'image de flotter, elle est positionnÃ©e en haut de la deuxiÃ¨me page, car il n'y a pas de place pour elle au bas de la premiÃ¨re page. L'option `ht` influence l'endroit oÃ¹ LaTeX peut placer le flottant ; ces deux lettres signifient qu'il peut aller lÃ  oÃ¹ il se trouve dans la source (donc Ã  cÃ´tÃ© de `Emplacement initial`) ou en haut d'une page. Vous pouvez utiliser jusqu'Ã  quatre spÃ©cificateurs de position :
...
[Plus tard](lesson-09), nous verrons comment faire rÃ©fÃ©rence Ã  des flottants dans votre texte, afin que le lecteur puisse les trouver facilement mÃªme s'ils sont Ã©tÃ© dÃ©placÃ©s de quelques pages.

Vous avez sans doute remarquÃ© qu'ici nous avons centrÃ© l'image en utilisant la commande `\centering` plutÃ´t que l'environnement `center`. Ã€ l'intÃ©rieur d'un flottant, vous devez utiliser cette commande pour centrer horizontalement le contenu ; cela Ã©vitera que le flottant et l'environnement `center` n'ajoutent chacun un espace vertical supplÃ©mentaire disgracieux.


## Travaux pratiques

Essayez d'inclure une image que vous avez crÃ©Ã©e, en remplacement les images standards que nous avons utilisÃ©es dans la dÃ©monstration.

Explorez ce que vous pouvez faire en utilisant les options `height`, `width`, `angle` et `scale`.

Utilisez le package `lipsum` pour constuire un exemple assez long, puis placez des flotteurs en utilisant les diffÃ©rents spÃ©cificateurs de position. Comment les diffÃ©rents spÃ©cificateurs interagissent-ils entre eux ?
